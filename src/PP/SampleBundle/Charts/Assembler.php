...
class Assembler
{
   ...
   *
   * @return $event \PP\SampleBundle\Event\ChartEvent
   */
  private function setChartEvent($order_id, $machine_name, $survey_type, $drilldown = array())
  {
    ...
    if ($responses === FALSE) {
      $fqg = $this->em->getReference('PPSampleBundle:FirstQGroup', $order_id);
      $responses = $this->em->getRepository('PPSampleBundle:LimeSurveyResponse')->findByFirstqgroup($fqg, array('updated' => 'DESC'));
