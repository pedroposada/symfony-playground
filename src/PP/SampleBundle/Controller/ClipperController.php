...
namespace PP\SampleBundle\Controller;
...
use PP\SampleBundle\Entity\Repository\FirstQProjectRepository;
use PP\SampleBundle\Entity\FirstQGroup as FirstQGroup;
use PP\SampleBundle\Entity\FirstQProject as FirstQProject;
use PP\SampleBundle\Entity\FirstQProcessAction as FirstQProcessAction;
use PP\SampleBundle\Service\GoogleSpreadsheetService;
use PP\SampleBundle\Service\SurveyBuilderService;
use PP\SampleBundle\Utils\CountryFWSSO as CountryFWSSO;
use PP\SampleBundle\Security\User\FWSSOQuickLoginUser as FWSSOQuickLoginUser;
...
class ClipperController extends FOSRestController
{
  ...
  public function getClipperAutocompleteAction(ParamFetcher $paramFetcher)
  {
    ...
    $path = $kernel->locateResource("@PPSampleBundle/{$file}");
  ...
  public function getOrdersAction(Request $request)
  {
    ...
    if (!empty($user_id)) {
      $firstq_groups = $em->getRepository('\PP\SampleBundle\Entity\FirstQGroup')->findByUserId($user_id);
    }
    ...
    if (!empty($firstq_groups)) {
      ...
      foreach ($firstq_groups as $key => $firstq_group) {
        ...
        $lsresponses = $em->getRepository('PPSampleBundle:LimeSurveyResponse')->findByFirstqgroup($firstq_group);
  ...
  public function getOrdersAdminAction(Request $request)
  {
    ...
    try {
      ...
      if (!empty($status)) {
        $firstq_groups = $em->getRepository('\PP\SampleBundle\Entity\FirstQGroup')->findByState($params['state_codes'][$status]);
      }
      ...
      if (!empty($firstq_groups)) {
        ...
        foreach ($firstq_groups as $key => $firstq_group) {
          ...
          if ($params['state_codes'][$status] == $params['state_codes']['order_declined']) {
            ...
            $firstq_process = $em->getRepository('\PP\SampleBundle\Entity\FirstQProcessAction')
  ...
  {
    $em = $this->getDoctrine()->getManager();
    $firstq_group = $em->getRepository('PPSampleBundle:FirstQGroup')->find($uuid);
  ...
  public function postOrderProcessAction(ParamFetcher $paramFetcher)
  {
    ...
    try {
      ...
      $em = $this->getDoctrine()->getManager();
      $firstq_group = $em->getRepository('PPSampleBundle:FirstQGroup')->find($firstq_group_uuid);
  ...
  private function createFirstQProject($form_data_serialized, $gs_result_array, $firstq_group_uuid)
  {
    ...
    if (!empty($firstq_group_uuid)) {
      ...
      if (!$firstq_group) {
        ...
        $fqps = $em->getRepository('PPSampleBundle:FirstQProject')->findByFirstqgroup($firstq_group->getId());
        foreach ($fqps as $fqp) {
          $em->remove($fqp);
        }
      }
  ...
  function sendSecurityEmail() {
    ...
    $message = \Swift_Message::newInstance()
      ...
      ->setBody(
        $this->renderView(
          'PPSampleBundle:Emails:security.alerts.html.twig',
  ...
  private function sendConfirmationEmail($to = array('recipient@example.com'), $type = '', $sales_info = array(), $link = '')
  {
    ...
    $message = \Swift_Message::newInstance()
      ...
      ->setBody(
        $this->renderView(
          // src/PP/SampleBundle/Resources/views/Emails/confirmation.{order_type}.{user_type}.html.twig
          'PPSampleBundle:Emails:confirmation.' . $type . '.html.twig',
          array(
  ...
  public function redirectLimeSurveyAction($sid, $slug, $lang)
  {
    ...
    if( isset($response['expires']) && !is_null($response['expires']) ) {
      ...
      // display message
      return $this->render('PPSampleBundle:Clipper:maximum.html.twig', array('url'=>$quotafull_url));
    }
    else {
      // redirect
  ...
  public function debugAction(ParamFetcher $paramFetcher)
  {
    ...
    // $debug = 'any output';
    // $debug = dump($debug);
    return $this->render('PPSampleBundle:Clipper:debug.html.twig', array('debug' => $debug));
  }
  ...
  {
    $response = $this->render('PPSampleBundle:Clipper:exit.html.twig', array('token' => $paramFetcher->get('lstoken')));
  ...
  public function thankyouAction($fquuid)
  {
    ...
    $parameters_clipper = $this->container->getParameter('clipper');
    $em = $this->getDoctrine()->getManager();
    $firstq_project = $em->getRepository('PPSampleBundle:FirstQProject')->find($fquuid);
  ...
  public function contactusAction(ParamFetcher $paramFetcher)
  {
    ...
    $message = \Swift_Message::newInstance()
      ...
      ->setBody(
        $this->renderView(
          'PPSampleBundle:Emails:contactus.html.twig', // Twig template
          array(
