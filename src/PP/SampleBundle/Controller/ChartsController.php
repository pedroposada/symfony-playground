...
class ChartsController extends FOSRestController
{
  ...
  public function chartsAction(ParamFetcher $paramFetcher)
  {
    ...
    $survey_type = $em->getRepository('PPSampleBundle:FirstQGroup')->find($order_id)->getFormDataByField('survey_type');
    ...
    foreach ($map['machine_names'] as $machine_name) {
      try {
        ...
        $charts->add($this->container->get('twig')->render("PPSampleBundle:Charts:{$machine_name}.html.twig", $placeholders));
      } catch (Exception $e) {
        // Do something, maybe?
      }
    }

    return $this->render("PPSampleBundle:Charts:charts.html.twig", array('charts' => $charts));
  }
  ...
  public function chartsReactAction(ParamFetcher $paramFetcher)
  {
    ...
    return $this->render("PPSampleBundle:Charts:charts-react.html.twig", array(
      'order_id' => $order_id,
    ));
  }
